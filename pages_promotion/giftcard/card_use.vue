		...
		<view class="order-container" :class="{ 'safe-area': isIphoneX }">
			...
			<scroll-view scroll-y="true" class="order-scroll-container">
				...
				<template v-if="paymentData">
					<template v-if="paymentData.is_virtual">
						...
						<view class="mobile-wrap">
							...
							<view class="form-group">
								<text class="icon"><image :src="$util.img('public/uniapp/order/icon-mobile.png')" mode="widthFix"></image></text>
					...
					<view v-if="paymentData.system_form" class="system-form-wrap"><ns-form :data="paymentData.system_form.json_data" ref="form"></ns-form></view>

					<!-- 订单金额 -->
					<template v-if="calculateData">
						<view class="order-submit bottom-safe-area">
							<view class="submit-btn"><button type="primary" class="sava-btn" size="mini" @click="create()">立即提货</button></view>
						</view>
		...
		selectDeliveryType(data) {
			...
			if (data.name == 'store' || data.name == 'local') {
				...
				if (!this.orderCreateData.member_address)
					this.orderCreateData.member_address = { mobile: this.paymentData && this.paymentData.member_account.mobile ? this.paymentData.member_account.mobile : '' };
			}
