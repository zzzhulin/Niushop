					...
					<block v-if="refundList.length">
						<view class="order-item" v-for="(item, index) in refundList" :key="index">
							<view class="order-header">
								<text class="status-num">{{ item.order_no }}</text>
								<view class="status-name" v-if="item.refund_status == 3">退款成功</view>
								<view class="status-name ns-text-color" v-if="item.refund_status == 1">退款中</view>
								<view class="status-name ns-text-color" v-if="item.refund_status == -1">退款失败</view>
							</view>
							<view class="goods-wrap">
								<image :src="$util.img(item.sku_image, { size: 'mid' })" @error="imageError(index)" mode="aspectFill" :lazy-load="true"></image>
								<view class="goods-info">
									<view class="goods-name" @click="refundDetail(item.id)">{{ item.sku_name }}</view>
									<view class="goods-num">
										<view class="num-text ns-text-color">{{ item.refund_status_name }}</view>
							...
							<view class="goods-btn">
								<view class="btn-text">
									...
									<text>退款：￥{{ item.presale_deposit_money }}</text>
								</view>
								<view class="order-operation">
									<view class="order-box-btn" @click="refundDetail(item.id)">查看详情</view>
									<!-- <block v-if="item.refund_action.length">
										<view class="order-box-btn" @click="refundAction(actionItem.event, item)" v-for="(actionItem, actionIndex) in item.refund_action" :key="actionIndex">
											{{ actionItem.title }}
										</view>
									</block> -->
								</view>
	...
	onShow() {
		if (!uni.getStorageSync('token')) {
			this.$util.redirectTo('/pages/login/login', {
				back: '/pages_promotion/presale/activist'
			});
		}
	},
	methods: {
		...
		refundDetail(order_goods_id) {
			this.$util.redirectTo('/pages_promotion/presale/refund_detail', {
