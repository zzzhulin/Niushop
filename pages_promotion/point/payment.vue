				...
				<view class="address-box" :class="{'not-delivery-type': orderPaymentData.express_type.length <= 1}" v-if="orderPaymentData.delivery.delivery_type != 'store'">
					...
					<view class="info-wrap" :class="{'local': orderPaymentData.delivery.delivery_type == 'local'}" v-if="orderPaymentData.member_address" @click="selectAddress">
						<view class="content">
							...
							<text class="font-size-base mobile">{{ orderPaymentData.member_address.mobile ? orderPaymentData.member_address.mobile : '' }}</text>
					...
					<block v-if="orderPaymentData.delivery.delivery_type == 'local'">
						<view class="local-box" v-if="orderPaymentData.local_config && orderPaymentData.local_config.info && orderPaymentData.local_config.info.time_is_open == 1">
