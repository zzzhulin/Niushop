	...
	<view class="member-level">
		<block v-if="levelList.length">
						...
						<view class="image-container" :class="[
							...
							<view class="slide-image" style="background-size: 100% 100%;background-repeat:no-repeat"
								...
								<image v-if="levelList[curIndex]['level_picture']"
									:src="$util.img(levelList[curIndex]['level_picture'])"></image>
					...
					<view class="card-time-list">
						<view class="card-item-box" :class="{ small: currCard.charge_rule_arr.length == 4 }"
							...
							<view class="card-time-item" :class="{ active: choiceIndex == index }"
								...
								<view class="time-origin-price">
									原价{{ $lang('common.currencySymbol') }}
									<text class="price">{{ item.original_price }}</text>
								</view>
								<view class="time-price">
									{{ $lang('common.currencySymbol') }}
									<text class="price">{{ item.sales_price }}</text>
									/{{ cardType[item.rule_type].unit }}
								</view>
								<view class="time-extra-text" v-if="item.consume_spending_num>0">
									<view>
										<text>消费限额{{ $lang('common.currencySymbol') }}</text>
										<text>{{ item.consume_spending_num }}</text>
									</view>
									<text>限额满后本卡会提前失效</text>
								</view>
								<view class="time-extra-text" v-else>
									<view>
										<text>不限消费额</text>
									</view>
									<text>
										送{{item.stock_num}}张体验卡
									</text>
								</view>
							</view>
						</view>
				...
				<view class="card-content"
					...
					<view v-if="currCard.send_coupon != '' || currCard.send_point > 0 || currCard.send_balance > 0">
						...
						<view class="card-privilege-list">
							...
							<view class="card-privilege-item" v-if="currCard.charge_rule_arr[choiceIndex].stock_num != ''">
								<view class="card-privilege-icon"><text class="iconfont icon-huiyuan"></text></view>
								<view class="card-privilege-name">会员卡礼包</view>
								<view class="card-privilege-text">赠送{{ currCard.charge_rule_arr[choiceIndex].stock_num }}张PLUS会员体验卡
								</view>
							</view>
						</view>
					</view>
				...
				<block v-if="currCard.charge_rule_arr.length">
					...
					<view class="action" :class="{ 'bottom-safe-area': isIphoneX, 'have-agreement': agreement }">
						...
						<!-- 	<text class="bold">{{ currCard.charge_rule_arr[choiceIndex].value }}</text>
							...
							<text class="bold">{{ currCard.charge_rule_arr[choiceIndex].sales_price }}</text>
							<text>/{{ cardType[currCard.charge_rule_arr[choiceIndex].rule_type].unit }}</text>
						</view>
						<view class="agreement" v-if="agreement">
							购买既视为同意
							<text
								@click="$util.redirectTo('/pages_tool/member/card_agreement')">《{{ agreement.title }}》</text>
						</view>
					</view>
				</block>
			</view>
			...
			<!-- <ns-payment ref="choosePaymentPopup" :payMoney="currCard.charge_rule_arr[choiceIndex].value"
				...
				<ns-payment ref="choosePaymentPopup" :payMoney="currCard.charge_rule_arr[choiceIndex].sales_price"
					@confirm="toPay" v-if="currCard.charge_rule_arr.length"></ns-payment>
		</block>
		<block v-else>
			<ns-empty text="暂无可开会员卡" :isIndex="false"></ns-empty>
		</block>
		...
		computed: {
			...
			currCard() {
				if (this.levelList[this.curIndex]) {
					...
					// });
					card.charge_rule_arr = card.rule_spec_list
					return card;
				}
			},
			storeToken() {
