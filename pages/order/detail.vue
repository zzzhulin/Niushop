			...
			<view class="store-wrap">
				...
				<view class="pick-block first-pick-block" v-if="">
					...
					<block v-if="orderData.buyer_ask_delivery_time && orderData.buyer_ask_delivery_time != 0" >
						<input class="last-child" disabled :value="orderData.buyer_ask_delivery_time.indexOf('-') != -1 ? orderData.buyer_ask_delivery_time : $util.timeStampTurnTime(orderData.buyer_ask_delivery_time)" />
					</block>
					<block v-else>
						<input class="last-child" disabled :value="'立即自提'" />
					</block>
				</view>
			</view>
		...
		<view class="order-summary">
			...
			<block v-if="orderData.goods_class == 5">
				<view class="order-cell">
					...
					<view class="box">
						<text class="color-base-text" @click="$util.redirectTo('/pages_promotion/cardservice/card/my_card')">查看</text>
					</view>
				</view>
			</block>
			<!-- 联系客服 -->
